from shiny import App, reactive, ui, req
from shinywidgets import output_widget, render_widget
from ipywidgets import Layout
from shapely.geometry import Point
import ipyleaflet as L
import geopandas as gpd
import json
import pickle
import os
import re

# Import your modules
import analysis
import preprocessing
import graph_builder

LAND_GDF = gpd.read_file("data/metro_vancouver_land_poly.geojson")

# =====================
# UI
# =====================

# Page Layout
app_ui = ui.page_sidebar(
    
    ui.sidebar(
        ui.h4("Settings", style="margin-top: 0; font-weight: bold;"),
        ui.input_select("day", "Select Day", choices=["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]),
        ui.input_text(
            "start_time", 
            "Select Start Time:", 
            value="17:00", 
            placeholder="HH:MM"
        ),
        ui.input_slider("budget", "Time Budget", 5, 60, 30),
        ui.input_slider("frequency", "Frequency Modifier", 0.1, 3.0, 1.0, step=0.1),
        ui.input_slider("walk_speed", "Walk Speed (m/s)", 0.5, 2.5, 1.2, step=0.1),
        ui.input_slider("max_walk", "Max Walk Distance (km)", 0.1, 3.0, 1.0, step=0.1),
        ui.input_checkbox_group("toggles", "Infrastructure Toggles (not functional yet)", 
                                {"bus": "Bus Routes", "skytrain": "SkyTrain", "bridges": "Bridges"}, 
                                selected=["bus", "skytrain", "bridges"]),
        ui.input_action_button("submit", "Update Settings", class_="btn-primary"),
    ),

    ui.head_content(
        ui.tags.style("""
            #custom-loading-overlay {
                position: fixed;
                top: 0; left: 0; width: 100%; height: 100%;
                background-color: rgba(0, 0, 0, 0.6);
                color: white;
                display: flex;
                justify-content: center;
                align-items: center;
                font-size: 24px;
                font-weight: bold;
                z-index: 99999;
                backdrop-filter: blur(2px);
                cursor: wait;
                opacity: 0;
                pointer-events: none;
                transition: opacity 0.2s ease-in-out 0.2s;
            }

            html.shiny-busy #custom-loading-overlay {
                opacity: 1;
                pointer-events: auto; 
            }
                      
            .leaflet-container,
            .leaflet-interactive {
                cursor: crosshair !important;
            }
            .leaflet-dragging, 
            .leaflet-dragging .leaflet-grab, 
            .leaflet-dragging .leaflet-interactive {
                cursor: grabbing !important; 
            }

            }
        """)
    ),

    ui.div("Calculating Connectivity...", id="custom-loading-overlay"),
    
    ui.card(
        ui.card_header(
            ui.span(
                "Metro Vancouver Transit Pulse – Transit Connectivity Simulator", 
                style="font-size: 24px; font-weight: bold;"
            ),
            class_="py-3" 
        ),
        output_widget("map_display", width="100%", height="100%"), 
        style="padding: 0; height: 90vh"
    ),
)


# =====================
# SERVER
# =====================

def server(input, output, session):
    
    selected_coords = reactive.Value(None)
    cache_state = {"last_day": None}

    # Initialize Map
    map_obj = L.Map(center=(49.21340119048903, -122.93785360348627), zoom=11, layout=Layout(height='100%') )
    map_obj.add_layer(L.basemaps.CartoDB.Positron)
    
    # Initialize User Marker
    user_marker = L.Marker(location=(0,0), draggable=False, visible=False)
    map_obj.add_layer(user_marker)

    # Handle Clicks
    def handle_click(**kwargs):
        if kwargs.get('type') == 'click':
            coords = kwargs.get('coordinates')
            print(f"📍 CLICK DETECTED: {coords}")
            
            user_marker.location = coords
            user_marker.visible = True
            
            selected_coords.set(coords)
            
    map_obj.on_interaction(handle_click)

    @render_widget
    def map_display():
        return map_obj

    # DATA LOADING === LINKS TO PREPROCESSING
    @reactive.Calc
    def get_network_data():
        """
        Triggered by 'Submit'.
        Checks if the day changed. If so, runs preprocessing.
        Always returns the pickle data.
        """
        input.submit()
        
        with reactive.isolate():
            selected_day = input.day()

        if selected_day != cache_state["last_day"]:
            print(f"🔄 Day changed to {selected_day}. Processing Network CSVs...")
            
            day_map = {
                "Monday": 1, "Tuesday": 1, "Wednesday": 1, "Thursday": 1, 
                "Friday": 1, "Saturday": 2, "Sunday": 3
            }
            
            preprocessing.process_network(day_id=day_map[selected_day])
            
            cache_state["last_day"] = selected_day
        else:
            print(f"✅ Day is still {selected_day}. Using cached Pickle.")
        
        if os.path.exists('data/network_edges.pkl'):
            with open('data/network_edges.pkl', 'rb') as f:
                return pickle.load(f)
        return None

    # GRAPH BUILD
    @reactive.Calc
    def current_graph():
        """
        Builds the graph using the loaded network data and current settings.
        """
        network_edges = get_network_data()
        req(network_edges)

        with reactive.isolate():
            time_str = input.start_time()
            freq_mod = input.frequency()
            # active_toggles = input.toggles() 

        if not re.match(r"^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$", time_str):
            print("Invalid format or time. Please use HH:MM (e.g., 14:30).")
            return
        
        print(f"Building Graph: {time_str} | Freq: {freq_mod}")
        
        return graph_builder.build_graph(
            current_time_str=time_str,
            window_mins=60,
            frequency_modifier=freq_mod
        )

    # ANALYSIS
    @reactive.Calc
    def isochrone_gdf():
        """
        Runs when:
        1. Graph updates (via Submit)
        2. Map is Clicked (via selected_coords)
        """
        # Dependencies that trigger updates
        G = current_graph()
        coords = selected_coords.get()
        
        # Stop if no click yet
        req(coords)
        
        with reactive.isolate():
            budget = input.budget()
            speed = input.walk_speed()
            max_walk = input.max_walk()
            
        print(f"Calculating Isochrone for {coords} with budget {budget}")
        if not LAND_GDF.contains(Point(coords[1], coords[0])).any():
            print("Error: Those coordinates are outside Metro Vancouver or not on land.")
            return
        else:
            try:
                return analysis.get_isochrone(
                    G=G,
                    start_lat=coords[0],
                    start_lon=coords[1],
                    time_budget_mins=budget,
                    walk_speed_mps=speed,
                    max_walk_km=max_walk
                )
            except Exception as e:
                print(f"Error in isochrone calc: {e}")
                return None

    # MAP UPDATE
    @reactive.Effect
    def update_map():
        gdf = isochrone_gdf()
        if gdf is None or gdf.empty: 
            for layer in map_obj.layers:
                if layer.name == 'isochrone':
                    map_obj.remove_layer(layer)
            return

        # DEBUG
        gdf.to_file('data/isochrone.geojson', driver="GeoJSON")

        new_layer = L.GeoJSON(
            data=json.loads(gdf.to_json()), 
            name='isochrone',
            style={'color': '#2b8cbe', 'fillOpacity': 0.4, 'weight': 2}
        )

        for layer in map_obj.layers:
            if layer.name == 'isochrone':
                map_obj.remove_layer(layer)
        
        map_obj.add_layer(new_layer)
        
        if user_marker.visible:
            if user_marker in map_obj.layers: map_obj.remove_layer(user_marker)
            map_obj.add_layer(user_marker)

app = App(app_ui, server)